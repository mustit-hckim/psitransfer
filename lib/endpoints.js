...
// login page
app.get(`${ config.baseUrl }login`, (req, res, next) => {
  res.send(loginPage({ ...pugVars, lang: req.translations }));
});

app.post(`${ config.baseUrl }login`, (req, res, next) => {
...
app.use((req, res, next) => {
  if (req.url === config.baseUrl) {
    // login 시 uploadAppPath 로 이동시켜줘야함
    console.log("app-use-baseUrl")
    return res.redirect(config.loginAppPath);
  }
